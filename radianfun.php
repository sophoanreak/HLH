...
//makes circle
	...
	for($i=1;$i<=30*pi();$i+=1){
		//2*pi as radian
		imageline($image,$x1,$y1,($x1 + ($r*cos($i))),($y1+($r * sin($i))),$green);
	}
	
	//blue points on circumference 
	for($i=1;$i<=20*pi();$i+=1){
		//2*pi as radian
		imagearc($image,($x1 + ($r*cos($i))),($y1+($r * sin($i))),100,30,0,360,$green);
	}
	
//call image
	imagepng($image,"image/radianfun.png");
	imagedestroy($image);
	
	echo '<center><img src="image/radianfun.png"></center>';
