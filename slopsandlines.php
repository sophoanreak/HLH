<html>
			...
			color: black;tom: 4px;">
					<td colspan = 2>
					<b><font style="font-size:1.2em;color:green;">Algebra >> Graphing >> Points and Lines :</font></b><br>
					<p>
					Objective: Graph points and lines using xy cordinates<br>
					</p>
					<p>
	...
	*/
	...
	imagepng($image,"image/slopeandlines1.png");
	imagedestroy($image);
	
	echo '<center><img src="image/slopeandlines1.png"></center>';
...
?>				
			...
			<table width = 659px border = 0 cellpadding = 0 cellspacing = 0 style="#fff;border: 0px solid: #98bf21;">
				<tr style= "font-size: 1em;
			text-align: left;
			padding-top: 5px;
			padding-bot: 5px;
...
<spamg>//create scene</spamg>
	$width = 400;
	$height = 400;
	$image1 = imagecreate($width,$height);
	
	<spamg>//Three colors, white, black and grey</spamg>
	$grey = imagecolorallocate($image1,204,204,204);
	$black = imagecolorallocate($image1,0,0,0);
	$blackAlpha = imagecolorallocatealpha($image1,0,0,0,50);
	...
	$yellow = imagecolorallocate($image1,240,240,19);
	
	<spamg>//make the background black</spamg>
	imagefill($image1,0,0,$blackAlpha);
	
	imageline($image1,0,$height/2,$width,$height/2,$grey);
	imageline($image1,$width/2,0,$width/2,$height,$grey);
	
	<spamg>//loop for the grid</spamg>
	for($i=10;$i&lt;400;$i+=5){ 
		imageline($image1,198,$i,202,$i,$grey);
	}
	for($i=5;$i&lt;395;$i+=5){ 
		imageline($image1,$i,198,$i,202,$grey);
	}
	
	<spamg>//making an arrow head for X and Y coordinate</spamg>
	imageline($image1,195,5,200,0,$white);
	imageline($image1,205,5,200,0,$white);
	imageline($image1,395,195,400,200,$white);
	imageline($image1,395,205,400,200,$white);
	...
	$xy1 = '('.($x1-200).','.(200-$y1).')';
	imagestring($GLOBALS['image1'],10,$x1,$y1,$xy1,$GLOBALS['green']);
	
	<spamg>//insert value of x2 and y2 on the graph</spamg>
	$xy2 = '('.($x2-200).','.(200-$y2).')';
	imagestring($GLOBALS['image1'],10,$x2,$y2,$xy2,$GLOBALS['green']);
	
	imagedashedline($GLOBALS['image1'],$x1,$y1,$x1,200,$GLOBALS['grey']);
	imageline($GLOBALS['image1'],$x1,$y1,200,$y1,$GLOBALS['grey']);
	
	imagedashedline($GLOBALS['image1'],$x2,200,$x2,$y2,$GLOBALS['grey']);
	imageline($GLOBALS['image1'],200,$y2,$x2,$y2,$GLOBALS['grey']);
	
	imageline($GLOBALS['image1'],$x1,$y1,$x2,$y2,$GLOBALS['green']);
	}
	
	<spamg>//call function line</spamg>
	line1(-20,-40,100,50);
	line1(-30,40,-150,150);
	
	imagepng($image1,"test1.png");
	imagedestroy($image1);
