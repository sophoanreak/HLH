<html>
			...
			color: black;tom: 4px;">
					<td colspan = 2>
					<b><font style="font-size:1.2em;color:green;">Geometry >> Parabola >> Y = 1/X :</font></b><br>
					<p>
					<b>Objective: Drawing Y = 1/X on PHP</b>
					</p>
					<p>
...
//below 200s is to coordinate (x,y) at the center
	$scale = 30;
	//Negative-side parabola
	
	for($temp=-0.1;$temp>=-10;$temp-=0.1){
		${'x'.($temp*-10)} = $temp *$scale;	
		${'y'.($temp*-10)} = (1/$temp)*$scale;
		}

	for($temp=-0.1;$temp>=-7;$temp-=0.1){
...
//call image
	imagepng($image,"image/yonex.png");
	imagedestroy($image);
	
	echo '<center><img src="image/yonex.png"></center>';
...
<spamg>//below 200s is to coordinate (x,y) at the center</spamg>
	$scale = 30;
	<spamg>//Negative-side parabola</spamg>
